...
async def book_appointment(
    ...
    try:
        ...
        doctor_location = appointment.doctorLocation
        ...
        try:
            background_tasks.add_task(
                send_appointment_confirmation, current_user["email"], appointment_data
            )
            logger.info(
                f"📧 Appointment confirmation queued for {current_user['email']}"
            )
        except Exception as e:
            logger.warning(
                f"Email failed to queue (ignoring to keep booking valid): {e}"
            )
...
async def get_appointments(current_user: dict = Depends(get_current_user)):
    ...
    try:
        cursor = db.appointments.find({"user_email": current_user["email"]}).sort(
            "created_at", -1
        )
...
async def cancel_appointment(
    ...
    try:
        ...
        query = {"_id": oid, "user_email": current_user["email"]}

        # Find and delete
        appointment = await db.appointments.find_one_and_delete(query)
        ...
        try:
            background_tasks.add_task(
                send_appointment_cancellation, current_user["email"], appointment
            )
            logger.info(f"📧 Cancellation email queued for {current_user['email']}")
        except Exception:
            pass  # Ignore email errors on cancel

        return standard_response(message="Appointment cancelled successfully.")
